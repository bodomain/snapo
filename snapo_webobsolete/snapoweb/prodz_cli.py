...
def show_menu():
    while True:
        ...
        print("4. Merge with another Database")
        print("5. Quit")
        ...
        if choice == "1":
            ...
            foreign_db = input("Enter the path to the foreign database: ").strip()
            if foreign_db:
                database.merge_db(foreign_db)
            else:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        parser.add_argument(
            ...
            "--merge",
            type=str,
            help="Path to another database to merge with",
        )
        args = parser.parse_args()

        if args.merge:
            database.merge_db(args.merge)
        else:
            start_prodz(
                args.work,
                getattr(args, "break"),
